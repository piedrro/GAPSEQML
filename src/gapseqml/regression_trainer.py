...
class Trainer:
    ...
    def train_step(self):
        ...
        for i, (images, labels) in batch_iter:
            ...
            accuracy = self.mean_absolute_error(labels,pred_label)
            ...
            batch_iter.set_description(
                f'Training: (Epoch: {self.epoch}/{self.epochs} loss: {np.mean(train_losses):.3f}, MAE: {np.mean(train_accuracies):.2f} LR: {current_lr})')  # update progressbar
    ...
    def val_step(self):
        ...
        for i, (images, labels) in batch_iter:
            ...
            with torch.no_grad():
                ...
                batch_iter.set_description(
                    f'Validation: (Epoch: {self.epoch}/{self.epochs} loss: {np.mean(valid_losses):.3f}, MAE: {np.mean(valid_accuracies):.2f} LR: {current_lr})')  # update progressbar
    ...
    def plot_batch(self, data, labels):
        ...
        for i in range(4):
            ...
            axes_label = labels[i]
            label = f"pred bound_fraction:{axes_label}"
    ...
    def evaluate_json_dataset(self, evaluation_dataset, model_path = None, sensitive = False):
        ...
        if model_path is not None:
            ...
            for i, localisation_data in enumerate(batch_iter):
                ...
                with torch.no_grad():
                    pred_labels = self.model(data)
                    
                    pred_labels = pred_labels.cpu().numpy()
                    pred_labels = np.squeeze(pred_labels)
                    
                    
                    if np.sum(pred_labels > 0.2) == 2:
                        
                        self.plot_batch(data, pred_labels)
